...
const CooldownTimer = () => {
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-0.5 sm:gap-1 lg:gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-1.5 sm:px-2 lg:px-3 py-1 sm:py-1.5 lg:py-2">
        {/* Timer Display */}
        <div className="flex items-center gap-0.5 sm:gap-1 lg:gap-2">
          <div className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 flex items-center justify-center">
            {isActive ? (
              ...
              <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-4 lg:h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col">
            <div className={`font-mono text-xs lg:text-sm font-semibold ${getTimerColor()}`}>
              {formatTime(timeLeft)}
            </div>
            {isActive && (
              <div className="w-10 sm:w-12 lg:w-16 h-0.5 sm:h-0.5 lg:h-1 bg-slate-200 dark:bg-slate-600 rounded-full overflow-hidden">
                <div 
        ...
        <div className="flex items-center gap-0.5">
          {!isActive ? (
            <button
              onClick={startTimer}
              className="px-1 sm:px-1.5 lg:px-2 py-0.5 lg:py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded transition-colors"
              title={`Start ${duration}-minute cooldown timer`}
            >
              <span className="hidden lg:inline">Start {duration}m</span>
              <span className="lg:hidden">{duration}m</span>
            </button>
          ) : (
            <button
              onClick={stopTimer}
              className="px-1 sm:px-1.5 lg:px-2 py-0.5 lg:py-1 text-xs bg-slate-500 hover:bg-slate-600 text-white rounded transition-colors"
              title="Stop timer"
            >
              Stop
            </button>
          ...
          <button
            ...
            className="p-0.5 lg:p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"
